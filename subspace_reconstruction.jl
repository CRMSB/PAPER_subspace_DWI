...
## Input : 
    ...
    - `params::Dict{Symbol,Any}` : number of iteration, regularization parameter value for wavelet
    - `bart_path::AbstractString` : path to the BART executable library
## Output :
    - `basis` : Matrix of size (Diffusion Direction,NUM_BASIS)
...
function subspace_bart_reconstruction(acq::AcquisitionData,params::Dict{Symbol,Any},bart_path::AbstractString)
    ...
    im_SUB = bart(1,"pics -d5 -S -e -i $(params[:iterations]) -R W:7:0:$(params[:λ])",k_bart,sens,B = basis);
